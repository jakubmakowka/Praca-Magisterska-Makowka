...
try {
    ...
    // Pobranie raportów
    $reports = [];
    $result = $conn->query("SELECT report_id, title, status, code, published_date, file_path FROM reports ORDER BY published_date DESC");
    if ($result) {
...
<head>
    ...
    <title>Fundacja Makówka - Sprawozdania</title>
...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 bg-slate-900 fixed-start" id="sidenav-main">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
            ...
            <div class="container-fluid py-1 px-2">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-1 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Sprawozdania</li>
                    </ol>
        ...
        <div class="container-fluid py-4 px-5">
            ...
            <div class="row">
                <div class="col-md-4">
                    <h3 class="font-weight-semibold mb-1">Sprawozdania finansowe</h3>
                    <p class="text-sm">Pobierz wszystkie dostępne sprawozdania finansowe fundacji:</p>
                </div>
            </div>
                    ...
                    <div class="card shadow-xs border mb-4">
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <form id="bulkDownloadForm" method="post" action="download_reports.php">
                                <table class="table align-items-center mb-0" id="reportsTable">
                                        ...
                                        <tr>
                                            <th class="py-3 px-4 text-start">
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <span class="h6 mb-0">Nazwa</span>
                                                </div>
                                            </th>
                                            <th class="text-start h6">Status</th>
                                            <th class="text-start h6">Kod</th>
                                            <th class="text-start h6">Opublikowano</th>
                                            <th class="text-start h6">Akcja</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php if (empty($reports)): ?>
                                            <tr><td colspan="5" class="text-center py-4">Brak dostępnych raportów.</td></tr>
                                        <?php else: ?>
                                            <?php foreach ($reports as $report): ?>
                                                <tr>
                                                    <td class="py-3 px-4">
                                                        <div class="d-flex align-items-center">
                                                            <div class="form-check mb-0">
                                                                <input class="form-check-input report-checkbox" type="checkbox" name="report_ids[]" value="<?= htmlspecialchars($report['report_id']) ?>">
                                                            </div>
                                                            <span class="font-weight-semibold text-dark ms-2"><?= htmlspecialchars($report['title']) ?></span>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <span class="badge badge-sm border <?= $report['status'] === 'Opublikowano' ? 'border-success text-success bg-success' : 'border-warning text-warning bg-warning' ?> border-radius-sm">
                                                    ...
                                                    <td><span class="text-sm"><?= htmlspecialchars($report['code']) ?></span></td>
                                                    <td><span class="text-sm"><?= date('d.m.Y', strtotime($report['published_date'])) ?></span></td>
                                                    <td>
                                                        <a href="<?= htmlspecialchars($report['file_path']) ?>" download class="text-sm font-weight-semibold text-dark">Pobierz</a>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                                        <?php endif; ?>
                                    ...
                                    <?php if (!empty($reports)): ?>
                                        <div class="bulk-actions p-3">
                                            <button type="submit" class="btn btn-sm btn-primary" id="bulkDownloadBtn" disabled>Pobierz wybrane</button>
                                        </div>
                                    <?php endif; ?>
                                </form>
                            </div>
    ...
    <script>
        // Inicjalizacja Scrollbar dla Windows
        ...
        selectAll.addEventListener('change', function() {
            ...
            bulkDownloadBtn.disabled = !this.checked && !Array.from(checkboxes).some(cb => cb.checked);
        });

        checkboxes.forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                bulkDownloadBtn.disabled = !Array.from(checkboxes).some(cb => cb.checked);
